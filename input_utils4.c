...
/*   Updated: 2022/07/30 16:38:51 by marvin           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	store_s(char *line)
{
	...
	line = skip_blanks(line);
	g_game_data.paths.s = ft_strtrim(line, " \t");
	if (!g_game_data.paths.s)
		return (-1);
	else
...
{
	line = skip_blanks(line);
	g_game_data.paths.s1 = ft_strtrim(line, " \t");
	if (!(g_game_data.paths.s1))
		return (-1);
	return (1);
}
...
{
	line = skip_blanks(line);
	g_game_data.paths.s2 = ft_strtrim(line, " \t");
	if (!(g_game_data.paths.s2))
		return (-1);
	return (1);
}
